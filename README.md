...
- **Multiple Prediction Modes**:
  - **Winner Prediction**: Traditional White/Draw/Black buttons
  - **Exact Score Prediction**: Choose between specific score options with configurable granularity
...
- **Centralized Theming System**: Unified styling architecture with multiple app skins
- **Layout Adaptability**: Responsive design supporting vertical and horizontal layouts
...
│   │   ├── global_configuration.dart # Global app configuration
│   │   ├── app_skin.dart            # App skin definitions
│   │   ├── layout_type.dart         # Layout type enumeration
│   │   ├── prediction_type.dart     # Prediction mode enumeration
│   │   ├── positioned_score_options.dart # Exact score prediction options
...
│   │   └── score_utils.dart         # Score parsing and formatting utilities
│   ├── themes/                      # Centralized theming system
│   │   ├── element_registry.dart    # UI element catalog and types
│   │   ├── style_definitions.dart   # Style properties and responsive dimensions
│   │   ├── unified_theme_provider.dart # Central theme provider
│   │   └── app_theme.dart           # Legacy theme (deprecated)
...
│       ├── exact_score_buttons.dart # Exact score prediction buttons
│       ├── adaptive_result_buttons.dart # Adaptive button wrapper
│       ├── score_display_buttons.dart # Score display widgets
│       ├── game_board_container.dart # Go board container with overlays
...
**Winner Prediction Mode:**
- **← Left Arrow**: Select White Wins
- **↓ Down Arrow**: Select Draw/Close
- **→ Right Arrow**: Select Black Wins

**Exact Score Prediction Mode:**
- **← Left Arrow**: Select left score option
- **↓ Down Arrow**: Select middle score option
- **→ Right Arrow**: Select right score option

**General:**
- **␣ Space Bar**: Pause Auto-Advance to Next Problem

### Configurable Options
- **Timer Control**: Enable/disable timer and configure duration per dataset
- **Prediction Type**: Choose between Winner Prediction and Exact Score Prediction modes
- **Score Granularity**: For exact score mode, set point difference between score options
- **Scoring Thresholds**: Configure what constitutes a "close" vs "decisive" win
- **Mark Display**: Enable/disable feedback marks and configure display duration
- **Dataset Selection**: Switch between different training datasets
- **App Skins**: Choose from Classic, Modern, Ocean, or E-ink themes
- **Layout Mode**: Switch between vertical and horizontal orientations
- **Auto-Advance**: Configure automatic progression to next problem
